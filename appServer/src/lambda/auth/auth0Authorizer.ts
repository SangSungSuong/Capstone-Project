...
const jwksUrl = 'https://sangvv.us.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  if (!signingKey) {
    throw new Error('Invalid with token:${jwt_}');
  }
...
function getToken(authHeader: string): string {
  if (!authHeader) throw new Error('Have not authentication header');
